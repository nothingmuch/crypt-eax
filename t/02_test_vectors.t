...
# uses test vectors from http://www.cs.ucdavis.edu/~rogaway/papers/eax.pdf
...
sub test_vector {
	...
	my $obj = Crypt::EAX->new(
		key => pack("H*",$args{KEY}),
		header => pack("H*",$args{HEADER}),
		nonce => pack("H*",$args{NONCE}),
	);

	my $ciphertext = $obj->encrypt( pack("H*", $args{MSG} ) );

	{
		is( uc(unpack("H*", $ciphertext)), $args{CIPHER}, "ciphertext matches" );
	}
	...
	lives_ok { $decrypted = $obj->decrypt( $ciphertext ) } "decryption lives";

	is( uc(unpack("H*", $decrypted ||'')), $args{MSG}, "decrypted" );

	foreach my $mangling ( [ 0, 5, "moose" ], [ -5, 3, "foo" ] ) {
		my $mangled = $ciphertext;
		substr($mangled, $mangling->[0], $mangling->[1] ) ^= $mangling->[2];
		dies_ok { $obj->decrypt($mangled) } "decryption fails";
	}
}
...
test_vector( %$_) for (
	...
	},
	{
		...
		'CIPHER' => '677C8D357B6712A7E4CF694D57626C80770A',
		'HEADER' => '',
		'MSG' => '3131',
		'NONCE' => ''
	}
